...
class PathClassificationTest(unittest.TestCase):
    ...
    def test_unknown_and_mod_paths_require_a_build(self) -> None:
        for path in (
            ...
            "src/main/resources/touhou_little_maid_mahjong.mixins.json",
            "src/main/resources/assets/touhou_little_maid_mahjong/lang/ja_jp.json",
            "src/main/resources/guide/README.md",
            "src/gametest/resources/data/touhou_little_maid_mahjong/structures/empty.nbt",
            "./src/gameTest/resources/README.md",
            r"src\main\resources\assets\touhou_little_maid_mahjong\README.md",
            "assets/logo.png",
...
class ArtifactReferenceTest(unittest.TestCase):
    def test_artifact_name_prefix_matches_upload_name(self) -> None:
        self.assertEqual(
            "touhou-little-maid-mahjong-mc-1.20.1-",
    ...
    def test_latest_matching_unexpired_branch_artifact_is_selected(self) -> None:
        artifacts = [
            ...
            {
                "id": 10,
                "name": "touhou-little-maid-mahjong-mc-1.20.1-old",
                "expired": False,
            ...
            {
                "id": 11,
                "name": "touhou-little-maid-mahjong-mc-1.20.1-expired",
                "expired": True,
            ...
            {
                "id": 12,
                "name": "touhou-little-maid-mahjong-mc-1.21.1-other",
                "expired": False,
            ...
            {
                "id": 14,
                "name": "different-mod-mc-1.20.1-newest",
                "expired": False,
                "created_at": "2026-05-01T00:00:00Z",
                "workflow_run": {"id": 104, "head_branch": "mc/1.20.1"},
            },
            {
                "id": 13,
                "name": "touhou-little-maid-mahjong-mc-1.20.1-latest",
                "expired": False,
    ...
    def test_skipped_build_summary_contains_artifact_link(self) -> None:
        ...
        artifact = ArtifactReference(
            13,
            "touhou-little-maid-mahjong-mc-1.20.1-latest",
            103,
...
class PushedHistoryTest(unittest.TestCase):
    ...
    def test_packaged_markdown_resource_requires_build(self) -> None:
        before = self.repository.commit_file("README.md", "initial\n", "initial")
        path = "src/main/resources/guide/README.md"
        after = self.repository.commit_file(path, "in-game guide\n", "guide resource")

        decision = detect_changes(self.repository.root, before, after)

        self.assertTrue(decision.mod_changed)
        self.assertEqual((path,), decision.relevant_paths)

    def test_source_renamed_to_documentation_requires_build(self) -> None:
        source_path = "src/main/java/example/Compat.java"
        before = self.repository.commit_file(source_path, "class Compat {}\n", "code")
        self.repository.git("config", "diff.renames", "true")
        self.repository.git("mv", source_path, "README.md")
        self.repository.git("commit", "-m", "move source into documentation")
        after = self.repository.git("rev-parse", "HEAD")

        decision = detect_changes(self.repository.root, before, after)

        self.assertTrue(decision.mod_changed)
        self.assertEqual((source_path,), decision.relevant_paths)
